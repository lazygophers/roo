...
1.  **[记忆库] 认知加载 (Cognitive Loading):**
    ...
    - **步骤 4: 精准加载本体:** 仅当摘要匹配成功，才加载该记忆的完整 `.yaml` 文件本体。
...
2.  **[沉淀] 提请中期持久化 (Propose Mid-term Persistence):**
    - **候选卡片生成**: **必须**通过 `ask_followup_question` 向用户提请审批“记忆候选卡片”，并建议其应存入的记忆层级 (L1 或 L2)。
    - 如果用户批准，将执行事务性压缩持久化流程。
...
1.  **[沉淀] 最终持久化 (Final Persistence):**
    ...
    - **事务性压缩持久化流程**:
        1.  **审批与分层**: 经用户批准，并确认记忆层级后，启动持久化流程。
        2.  **调用压缩**: 委托 `memory` 模式的 `summarize` 工作流。
        3.  **暂存**: 将处理后的记忆 `.yaml` 文件写入 `.memory/staging/`。
        4.  **更新摘要索引**: 调用 `memory` 模式的 `update_index` 工作流。
        5.  **确认与移动**: **仅在索引更新成功后**，将 `.yaml` 文件从 `staging` 移到目标层级目录。
