...
1.  **[记忆库] 认知加载 (Cognitive Loading):**
    ...
    - **步骤 4: 精准加载本体:** 仅当摘要匹配成功，才加载该记忆的完整 `.json` 文件本体（其中可能包含记忆指针）。
...
1.  **[复盘] 强制记忆复盘与提炼 (Mandatory Memory Review & Refinement):**
    ...
    -   **强制前置操作**: 在生成记忆卡片前，**必须**先读取 `.memory/README.md` 文件，确保产出的记忆严格遵循其中定义的最新字段和格式规范。
...
2.  **[沉淀] 提请中期持久化 (Propose Mid-term Persistence):**
    ...
    -   如果我认为有必要，你将执行事务性压缩持久化流程，将提炼出的知识沉淀到 L1 或 L2 记忆库中。这确保了在长周期父任务中，后续的子任务可以利用前面子任务的成果。
...
1.  **[沉淀] 最终持久化 (Final Persistence):**
    ...
    -   **事务性压缩持久化流程**:
        ...
        3.  **暂存**: 将处理后的记忆 `.json` 文件（包含指针）写入 `.memory/staging/` 目录。
        4.  **更新摘要索引**: 调用 `memory` 模式的 `update_index` 工作流，将新记忆的**摘要**和元数据更新到目标层级 (L1/L2) 的 `index.json` 中。
        5.  **确认与移动**: **仅在索引更新成功后**，将 `.json` 文件从 `.memory/staging/` 移动到对应的 `L1_declarative` 或 `L2_procedural` 目录中。
