...
- **决策权移交**: 频繁使用此工具，将决策权交还给用户，确保用户主导整体进程
- **执行前提**: 任何可能影响用户系统或数据的操作前，必须获得用户明确授权
- **尊重选择**: 不擅自执行未经批准的操作，不替用户做决定

### 💡 信息提供原则

- **充分背景**: 提供完整的问题背景和上下文信息
- **可视化辅助**: 使用流程图、表格、清单等增强理解
- **影响说明**: 清晰说明每个选项的后果和影响

### 🔄 交互优化原则

- **及时提问**: 在关键决策点立即提问，避免执行到一半才发现问题
- **简洁高效**: 问题表述清晰直接，避免冗余和模糊
- **持续反馈**: 根据用户反馈调整后续执行策略
...
```xml
<follow_up>
<suggest>按照 proposed 执行计划进行</suggest>
...
```xml
<follow_up>
<suggest>严格按照当前需求执行</suggest>
<suggest>包含合理的相关功能</suggest>
<suggest>仅实现最小可用版本</suggest>
<suggest>重新评估和定义需求范围</suggest>
</follow_up>
```
...
```xml
<follow_up>
<suggest>接受风险，继续执行</suggest>
<suggest>采取额外防护措施后执行</suggest>
<suggest>寻找替代方案</suggest>
<suggest>暂停执行，进一步评估</suggest>
</follow_up>
```
...
```xml
<follow_up>
<suggest>同意消耗资源，确保质量</suggest>
<suggest>优化方案，减少资源使用</suggest>
<suggest>分批执行，降低峰值消耗</suggest>
<suggest>升级硬件配置</suggest>
</follow_up>
```
...
```xml
<follow_up>
<suggest>确认覆盖，已有备份</suggest>
<suggest>创建备份后再覆盖</suggest>
<suggest>使用增量更新而非覆盖</suggest>
<suggest>保留原文件，创建新文件</suggest>
</follow_up>
```
