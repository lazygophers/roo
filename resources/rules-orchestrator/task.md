...
- **强制层级拆解**: 必须按照层级结构进行拆解，最少拆解到 2 级任务结构
...
- **最小层级**: 所有任务必须拆解到至少 2 级结构（主任务 → 子任务）
- **层级深度**: 最大不超过 2 级，确保任务结构清晰可控
...
**核心原则**：严格按照最小 2 级、最大 2 级的强制层级结构，使用点号编号（如 001.001），确保父子任务依赖明确且可追溯。
...
- **最小层级**: 任何任务必须拆解到至少 2 级结构
- **最大层级**: 严格控制不超过 2 级，避免过度复杂化
...
| 要求类型     | 强制标准                | 违规后果     |
| ------------ | ----------------------- | ------------ |
| **任务拆解** | 必须拆解到最小 2 级结构 | 禁止直接执行 |
| **层级编号** | 必须使用点号编号格式    | 任务无法管理 |
