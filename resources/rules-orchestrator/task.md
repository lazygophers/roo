...
2. **委托内容要求**
   ...
   - **任务上下文**: 必须包含 `task_id_list`、`metadata` 和完整背景信息
...
<new_task>
  ...
  <message>
    metadata:
      task_id: "当前任务ID"
      task_id_list: ["主任务ID", "父任务ID", "相关任务ID"]
      category: "任务分类"
      tags: ["标签1", "标签2"]
    task:
      description: "任务描述"
      ...
      boundaries:
        allowed_files: ["允许操作的文件"]
        disallowed_patterns: ["禁止的操作模式"]
      acceptance:
...
- **强制多层级拆解**: 必须按照层级结构进行深度拆解，最少拆解到3级任务结构
...
- **最小层级**: 所有任务必须拆解到至少3级结构（主任务 → 子任务 → 孙任务）
- **层级深度**: 最大不超过4级，确保任务结构清晰可控
- **拆解完整性**: 每个父任务必须包含至少2个子任务，避免单一路径
...
**核心原则**：严格按照最小3级、最大4级的强制层级结构，使用点号编号（如 001.001.001），确保父子任务依赖明确且可追溯。
...
- **最小层级**: 任何任务必须拆解到至少3级结构
- **最大层级**: 严格控制不超过4级，避免过度复杂化
- **子任务数量**: 每个父任务必须包含至少2个子任务
- **原子化要求**: 最终层级任务必须是原子化操作，可独立执行和验证
...
---
**文档版本**: v2.0 (强化多层级拆解和委托规范)
**最后更新**: 2025-09-06
**适用范围**: AI系统任务管理，特别是orchestrator模式的任务分解和委托