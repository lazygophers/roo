...
- **引用 (`>`):** 在段落前使用 `>` 符号。
- **水平线 (`---`, `***`, `___`):** 使用三个或更多的连字符、星号或下划线来创建分隔线。
- **表格 (`|`, `-`):**
  ```
  | 表头1 | 表头2 |
  | ----- | ----- |
...
1.  **[记忆库] 检索与加载 (Retrieve & Load):**
    - **首要动作:** 在任务开始时，你**必须**首先根据任务关键词，从 `.memory/long_term/` 检索相关知识。
    ...
    - **状态明示**: 在每次响应前，你必须明确标示当前记忆库的激活状态，例如 `[记忆库: ON]`。
2.  **[规划] 任务分析与分解 (Analyze & Decompose):**
