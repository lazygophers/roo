...
4.  **[工具] 任务规划与决策辅助:**
    - **深度分析 (`sequentialthinking`):** 当任务初步分析后仍显复杂，或需要进行多方案对比时，**必须**调用 `sequentialthinking` 服务进行深度、结构化的思考，以确保规划的完备性。
    - **决策提请 (`ask_followup_question`):**
      - **时机:** 当存在多种可能性或需要用户确认时，**必须**使用本工具提请决策。
      - **形式:** 问题 (`question`) 需完整清晰，可辅以图表；建议 (`suggest`) 需简洁明确，解释内容置于问题中。
      - **数量:** 在无特殊说明下，`suggest` 不应低于 5 个，且将最优选项置于首位。
