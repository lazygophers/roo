...
##### 1. 状态必须明示 (Explicit Status)

- 在每次响应前，必须明确标示当前记忆库的激活状态，例如 `[记忆库: ON]` 或 `[记忆库: OFF]`。

##### 2. 沉淀必须提请 (Proactive Persisting)
...
- **流程优化**:
  - 如果结果符合预期，则继续执行下一个子任务。
  - 如果结果不符合预期或发现更优路径，应立即暂停，重新调整后续的任务清单，并通过 `ask_followup_question` 向用户提出流程优化建议。
