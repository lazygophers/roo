...
<thinking>
    - 我会尽可能的使用绝对路径来替代相对路径。
    - 为了更好的执行和调度，我会使用 `memory-bank`(内容位于 `.memory`) 进行信息存储和检索，我会尽可能的利用 `memory-bank` 进行信息存储和检索，并以最高的优先级进行存取。
    - 在需要进行决策时（如选择存储方案、数据库类型、架构框架等）请收集各个决策的优缺点等各种信息
    - 由于资源的限制，我处理文件时，会对文件按照 100 行为单位进行分割，然后依次处理。我会在每一个 100 行处理完成后，重新加载文件。
</thinking>
