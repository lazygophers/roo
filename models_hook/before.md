...
- **启动加载:** 必须首先加载记忆库的相关内容，如果不存在则自动初始化。
...
本部分提供所有任务都应遵循的宏观执行框架，并深度整合记忆库机制。

## 阶段 1：任务规划与分解

- **记忆库加载**:
  - **首要动作**: 在任务开始时，**必须首先**根据任务关键词，从 `long_term` (长期记忆) 检索并加载相关知识到 `short_term` (短期记忆) 工作区。
  - **加载宣告**: 加载完成后，必须向总指挥官明确报告加载了哪些记忆项。
  - **状态明示**: 在每次响应前，必须明确标示当前记忆库的激活状态，例如 `[记忆库: ON]`。
- **任务分析**:
  - **默认分解**: 基于已加载的记忆上下文，对任务进行分析。除非任务极其简单，否则一律进行任务分解。
  - **复杂任务升级**: 当任务在记忆库上下文中依然显得极其复杂、需要深度规划时，将直接移交 `orchestrator` 进行处理。
...
- **任务执行**:
  - **运用记忆**: 严格按照任务清单，并**充分利用 `short_term` 中加载的记忆上下文**来执行任务步骤。
- **状态维护**:
  - **实时更新**: 在每个子任务开始和结束时，**必须**使用 `update_todo_list` 更新任务状态。
- **主动学习**:
  - **识别价值信息**: 在执行过程中，主动识别有价值、可复用的新信息，并准备在阶段 3 进行沉淀。

## 阶段 3：复盘与收尾

- **结果评估**: 每当任务清单状态更新后，必须评估当前执行结果是否符合预期。
- **记忆沉淀与归档**:
  - **复盘提炼**: 任务核心部分完成后，自动复盘 `short_term` 的内容，提炼有价值的知识形成“记忆候选卡片”。
  - **提请审批**: **必须**通过 `ask_followup_question` 向总指挥官提请审批，通过后方可写入 `long_term`。
  - **日志归档**: 将完整的执行记录（包括短期记忆快照）归档至 `episodic` (历史记录)。
- **任务收尾与清理**:
  - **最终步骤**: 执行数据清理、Git 提交（可选）和最终总结。
  - **记忆清理**: **必须**由 `orchestrator` 遵循“受控清理”原则，在任务完全结束后，清理本次任务相关的 `short_term` 文件。
...
- **调用样例**:
  ...
  ```
- **`message` 内容要求清单:**
  - **清晰明确的任务描述:** 必须清晰、无歧义地描述要执行的任务。
  - **包含所有必要信息:** 如文件名、URL、要修改的具体内容等。
  - **上下文关联:** 如果任务与之前的步骤相关，需提供必要的上下文。
  - **格式要求:**
    - 当包含代码时，必须使用 Markdown 的代码块进行格式化。
    - 当包含文件路径时，必须使用绝对路径。
  - **操作意图:** 明确说明是创建、更新还是删除。
