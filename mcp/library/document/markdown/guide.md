...
- **长列表懒编号**：列表可能变动时，使用 `1.` 统一编号（Markdown 会自动渲染正确序号），例如：
  ```markdown
  1. 步骤一  
  1. 步骤二  
      1. 子步骤一  
  ...
  1. 步骤三  
  ```  
- **短列表全编号**：固定且简短的列表（如 3 项以内）使用 `1.`、`2.` 按序编号。
...
1. **HTML 混用**：仅在必要时使用 HTML（如复杂表格），优先使用纯 Markdown。
2. **字符编码**：确保文件为 UTF-8 编码，避免特殊字符乱码。
3. **行尾格式**：统一使用 LF（Unix 风格）而非 CRLF（Windows 风格）。