...
class SearxChecker:
    ...
    def check_host_direct(self, host: str) -> CheckResult:
        ...
        try:
            ...
            params = {"q": self.test_query, "format": "json", "categories": "general", "pageno": 1}
            ...
            if isinstance(data, dict) and "results" in data:
                ...
                return CheckResult(host=host, is_alive=True, response_time=response_time)
            else:
                logger.debug(f"❌ {host} 响应格式无效")
                return CheckResult(host=host, is_alive=False, error="Invalid response format")

        except requests.exceptions.Timeout:
            logger.debug(f"⏱️ {host} 超时")
            return CheckResult(host=host, is_alive=False, error=f"Timeout after {self.timeout}s")
        except requests.exceptions.ConnectionError as e:
            logger.debug(f"🔌 {host} 连接失败: {e}")
            return CheckResult(host=host, is_alive=False, error=f"Connection error: {str(e)[:50]}")
        except Exception as e:
            logger.debug(f"❌ {host} 检测失败: {e}")
            return CheckResult(host=host, is_alive=False, error=f"Error: {str(e)[:50]}")
    ...
    def check_host_via_tool(self, host: str) -> CheckResult:
        ...
        try:
            ...
            # 使用 search 函数进行测试
            results = search(query=self.test_query, categories="general", page=1)
            ...
            if results and isinstance(results, dict) and "results" in results:
                ...
                return CheckResult(host=host, is_alive=False, error="No valid results")
    ...
    def check_hosts_concurrent(self, hosts: list[str], use_tool: bool = False) -> list[CheckResult]:
        """
        并发检测多个 Searx 实例
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # 提交所有任务
            future_to_host = {executor.submit(check_func, host): host for host in hosts}
    ...
    def print_summary(self, results: list[CheckResult]):
        ...
        # 性能统计
        if alive_results:
            avg_time = sum(r.response_time for r in alive_results if r.response_time) / len(
                alive_results
            )
...
class ConfigManager:
    ...
    def update_searx_hosts(self, hosts: list[str], dry_run: bool = False) -> bool:
        """
        ...
        try:
            ...
            if dry_run:
                ...
                print("\n将要写入的配置:")
                print(
                    yaml.dump({"searx_hosts": hosts}, allow_unicode=True, default_flow_style=False)[
                        :500
                    ]
                )
                return True
...
示例:
  ...
  %(prog)s --verbose                 # 显示详细日志
    ...
    parser.add_argument(
        "-q", "--query", type=str, default="test", help="测试查询字符串 (默认: test)"
    )
    ...
    parser.add_argument("--host-file", type=str, help="从文件读取主机列表（每行一个 URL）")

    parser.add_argument("--use-tool", action="store_true", help="使用项目的 searx 工具进行检测")

    parser.add_argument("--dry-run", action="store_true", help="模拟运行，不实际更新配置文件")

    parser.add_argument("-v", "--verbose", action="store_true", help="显示详细日志")

    parser.add_argument("--no-update", action="store_true", help="只检测，不更新配置文件")
